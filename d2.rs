const INPUT: &str = "104	240	147	246	123	175	372	71	116	230	260	118	202	270	277	292
...
fn one(input: &Vec<Vec<i32>>) -> i32 {
    let mut chsums: Vec<i32> = vec![];
    for line in input {
        let max: i32 = *line.iter().max().unwrap();
        let min: i32 = *line.iter().min().unwrap();
        chsums.push(max - min);
    }
    chsums.iter().sum()
}
...
fn two(input: &Vec<Vec<i32>>) -> i32 {
    ...
    for line in input.iter() {
        for i in line.iter() {
            let other = line.iter()
                .skip_while(|e| !(*i % **e == 0 && *i != **e))
                .nth(0);
            match other {
                Some(x) => divs.push(*i / x),
                None    => ()
            }
        }
    }
